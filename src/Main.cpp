...
struct alignas(8) TableDescriptor
{
    ...
    uint64_t output_addr : 28; // [39:12] Output address
    uint64_t reserved1   : 12; // [51:40] Reserved
    uint64_t ignored1    :  7; // [58-52]
    uint64_t pxnt        :  1; // [59]
    uint64_t xnt         :  1; // [60]
...
constexpr uint64_t MAIR_ATTR = 0xFF | (0x04 << 8);
...
void KernelMain()
{
    ...
    if (((el >> 2) & 0b11) == 2)
    {
        ...
        MMIO_BASE = 0xFF00'0000u; // Update MMIO base
...
void KernelMainX()
{
    // Set up EL1 stack pointer (SP_EL1)
    extern uint64_t StackBuffer[];
    asm volatile (
        "msr sp_el1, %0\n"
        :
        : "r"(&StackBuffer[0x10000/8])
    );

    // Set EL1 exception return address to KernelMain
    ...
    uint64_t spsr = (0b0101 << 6); // M[3:0]=0101 (EL1h), all interrupts enabled
    asm volatile (
        "msr spsr_el2, %0\n"
        :
        : "r"(spsr)
