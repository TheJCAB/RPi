...
void CallbackC(uintptr_t)
{
    ...
    Timer::ScheduleSpark(2000000, { CallbackA }); // 2 seconds = 2,000,000 microseconds
}
...
void DemonstrateScheduledTimers()
{
    ...
    Timer::SparkHandle handle_a = Timer::ScheduleSpark(1'000'000, { CallbackA });  // 1 second
    Timer::SparkHandle handle_b = Timer::ScheduleSpark(2'500'000, { CallbackB });  // 2.5 seconds
    Timer::SparkHandle handle_c = Timer::ScheduleSpark(5'000'000, { CallbackC });  // 5 seconds
    ...
    uint64_t future_time = current_time + Timer::MicrosecondsToTicks(3'000'000); // 3 seconds from now
    ...
    cancel_handle = Timer::ScheduleSpark(500'000, { NeverCallCallback });

    // Cancel the timer after 100ms
    Timer::ScheduleSpark(100'000, { CancelTimerCallback });
